...
impl Default for UnitCommander {
    fn default() -> Self {
        Self { 
            pushed_commands: Vec::new(), 
            attack_moves: Vec::new(), 
            pure_moves: Vec::new(), 
...
fn process_command_pushes(
    context: ResMut<UnitCommander>,
    q: Query<&mut Commandable>,
) {
...
fn process_command_core(
    mut context: ResMut<UnitCommander>,
...
fn process_pure_movement(
    command_core: CommandCore,
    context: &mut ResMut<UnitCommander>,
    q: &mut Query<&mut Commandable>,
){
    let commandable = q.get_mut(command_core.recieving_unit);
    let mut commandable = commandable.unwrap();
